...
public class StartupPool
{
    ...
    public StartupPool(GameObject _prefab, int _size)
    {
        prefab = _prefab;
        size = _size;
    }
...
public sealed class ObjectPool : MonoBehaviour
{
    ...
    // public GameObject listitem_item;
    // public GameObject[] uiPrefabs;
    public int preCreateSize = 0;
    ...
    public void InitStartupPools()
    {
        Debug.Log("OpjectPool.InitStartupPools()");
        List<StartupPool> _startupPools = new List<StartupPool>();

        foreach (ItemComponent itemComponent in PrefabManager.Instance.items)
        {
            _startupPools.Add(new StartupPool(itemComponent.gameObject, preCreateSize));
        }

        foreach (EnemyComponent enemyComponent in PrefabManager.Instance.enemies)
        {
            _startupPools.Add(new StartupPool(enemyComponent.gameObject, preCreateSize));
        }

        foreach (TrapComponent trapComponent in PrefabManager.Instance.traps)
        {
            _startupPools.Add(new StartupPool(trapComponent.gameObject, preCreateSize));
        }

        foreach (BlockComponent blockComponent in PrefabManager.Instance.blocks)
        {
            _startupPools.Add(new StartupPool(blockComponent.gameObject, preCreateSize));
        }

        startupPools = _startupPools.ToArray();
        CreateStartupPools();
    }
    ...
    {
        // Debug.Log("CreatePool(" + prefab.name + ", " + initialPoolSize.ToString() + ")");
